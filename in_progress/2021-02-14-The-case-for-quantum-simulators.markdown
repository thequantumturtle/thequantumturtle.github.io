---
layout: post
title: The Case For Quantum Simulators
date: 2021-02-14 01:39:00 -0500
description: With real quantum computers available to the public, do we need a simulators?
img: i-rest.jpg # Add image post (optional)
fig-caption: # Add figcaption (optional)
tags: [Quantum Computers, Simulators]
---
With Quantum Computers being realized in the last few years and with their accessibility to the general public growing on an almost daily basis it may be opaque to an individual not familiar with the space as to why we still rely and invest in Quantum Computer Simulators. QC simulators are extremely limited in their abilities and quickly become obsolete as the number of qubits grow beyond just a handful. 

To see how quickly the problem of simulating a QC on a classical computer (CC) explodes lets first look at a single qubit. Accurately representing a qubit requires 2 complex numbers, $$c=a+bi$$. Often this will be shown as  $$\left(\begin{smallmatrix} a_0 + b_0i \\ a_1 + b_1i \end{smallmatrix}\right)$$ Both $$a$$ and $$b$$ can be represented by floats and its understood that $$b$$ has an imaginary component, so we can just drop the $$i$$ and represent the qubit state like so: $$\left(\begin{smallmatrix} a_0 & b_0 \\ a_1 & b_1 \end{smallmatrix}\right)$$.  Assuming that each float is 64 bits, the number of bits required to represent a qubit using classical bits would look like so: 

<center>
$$\Large{
    \left(\begin{smallmatrix} 64 & 64 \\ 64 & 64 \end{smallmatrix}\right) 
    = 
    \left(\begin{smallmatrix} 128 \\ 128 \end{smallmatrix}\right) 
    = 
    \left(\begin{smallmatrix} 256 \end{smallmatrix}\right)
    }$$. 
</center>

Each row in the vector column is equivalent to a state
